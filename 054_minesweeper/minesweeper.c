...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  board_t * ptr_myBoard = malloc(sizeof(*ptr_myBoard));
  ptr_myBoard->board = malloc(sizeof(int *) * h);
  for (int i = 0; i < h; i++) {
    ptr_myBoard->board[i] = malloc(sizeof(int) * w);
  }
  for (int i = 0; i < h; i++) {
    for (int j = 0; j < w; j++) {
      ptr_myBoard->board[i][j] = UNKNOWN;
    }
  }

  ptr_myBoard->width = w;
  ptr_myBoard->height = h;
  ptr_myBoard->totalMines = numMines;
  
  for (int i = 0; i < numMines; i++) {
    addRandomMine(ptr_myBoard);
  }
...
int countMines(board_t * b, int x, int y) {
  ...
  int count = 0;
  int row_mask[8] = {-1, -1, -1, 0, 0, 1, 1, 1};
  int col_mask[8] = {-1, 0, 1, -1, 1, -1, 0, 1};
  for (int i = 0; i < 8; i++) {
    int n_x = x + col_mask[i];
    int n_y = y + row_mask[i];
    if (n_x >= 0 && n_x < b->width && n_y >= 0 && n_y < b->height) {
      if (IS_MINE(b->board[n_y][n_x])) {
        count++;
      }
    }
  }
