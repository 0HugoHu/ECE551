...
// Play the game
// Loop until reaches win or lose page
  ...
  while (type == "Normal") {
    ...
    if (isStep4) {
      pages[curr]->printPage(
          this->directory, pages[0]->varList, pages[0]->varValList, umap);
    }
    else {
...
// Backtracking algm for finding path to win
void GamePlay::backtracking(std::vector<std::string> & res,
  ...
  if (pages[pageIndex]->getChoice().size() == 0 ||
      pages[pageIndex]->choiceVisited[choiceIndex]) {
    return;
  }
  ...
  for (size_t j = 0; j < pages[pageIndex]->getChoice().size(); j++) {
    ...
    backtracking(res,
                 pages,
                 this->getIndexByPageNum(pages, pages[pageIndex]->getChoice()[j]),
                 0,
                 s + ss.str() + "(" + ss2.str() + "),");
  }
...
// Get choiceGoto with page number
...
// Put choiceGoto with page number
